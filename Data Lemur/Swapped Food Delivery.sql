...
**/
with mapping as (
SELECT *, lead(item) over (order by order_id) as even_row,
lag(item) over (order by order_id) as odd_row
FROM orders 
)

select order_id , case when order_id % 2= 0 then coalesce(odd_row,item)
when order_id % 2= 1 then coalesce(even_row,item) end as item
from mapping ;
